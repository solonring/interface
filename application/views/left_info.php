    ...
    <div class="form-group">
        <input type="text" class="form-control" id="searchcate" onkeyup="search('api',this)" placeholder="标题汉字或者拼音">
    </div>
    ...
    <div class="list">
        <ul class="list-unstyled" style="padding:10px">
            <?php foreach ($left as $key => $value) {?>
                        <li class="menu" id="api_<?php echo $value['id'];?>">
                <a href="<?php echo site_url('c=api&m=index&cid='.$_GET['cid']);?>#info_api_<?php echo $value['id'];?>" id="menu_<?php echo $value['id'];?>">
                    <span class="glyphicon glyphicon-menu-right" aria-hidden="true"></span>
                    <?php echo $value['title'];?></a> 
                    <?php if($this->session->userdata('level')==1){?><span style="color:#FF0000; cursor:pointer; float:right" onclick="deleteApi('<?php echo $value['id'];?>','<?php echo $value['id'];?>');">&nbsp;&nbsp;&nbsp;&nbsp;删除</span><?php }?>
                    <?php if($this->session->userdata('level')==1 || $this->session->userdata('level')==2){?><span style="color:#104E8B; cursor:pointer; float:right" onclick="editApi('<?php echo site_url('c=add&m=edit&id='.$value['id']).'&cid='.$_GET['cid'];?>');">编辑</span><?php }?>
            </li>
            <!--接口关键字(js通过此关健字进行模糊查找)start-->
            <span class="keyword" id="<?php echo $value['id'];?>"><?php echo Pinyin($value['title']);?> <?php echo $value['title'];?></span>
